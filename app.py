...
if query:
    ...
    if 'todos_links' in st.session_state and st.session_state.todos_links:
        ...
        if 'filter_stats' in st.session_state:
            ...
            st.info(f"🔍 Filtragem: {stats['processed']} processados | {stats['blocked']} spam removido | {stats['filtered']} livros")
        
        st.subheader(f"📖 Resultados (página {st.session_state.pagina + 1} de {(total-1)//RESULTADOS_POR_PAGINA + 1})")
        
        for idx, url in enumerate(links_pagina, 1):
            st.markdown(f"**{inicio + idx}.** [{url[:100]}...]({url})")
        
        st.markdown("---")
        col1, col2 = st.columns([2, 1])

        with col1:
            st.subheader("🗺️ Top Domínios")
